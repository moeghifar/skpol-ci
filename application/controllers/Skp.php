...
class Skp extends MY_Controller {
			...
			if(count($countError) == 0){
				...
				foreach($nama_produk as $kk => $vv) {
					...
					foreach($tangkap as $k => $i){
						...
						if($i != ""){
							$data['bahanbaku']['t'][$k]['asal_bahanbaku']=$this->parse_to_comma($i, '~');
							$data['bahanbaku']['t'][$k]['jenis_bahanbaku']=$this->parse_to_comma($jenis[$k], '~');
							$data['bahanbaku']['t'][$k]['bentuk_bahanbaku']=$this->parse_to_comma($bentuk[$k], '~');
					...
					foreach($budidaya as $k => $i){
						...
						if($i != ""){
							$data['bahanbaku']['b'][$k]['asal_bahanbaku']=$this->parse_to_comma($i, '~');
							$data['bahanbaku']['b'][$k]['jenis_bahanbaku']=$this->parse_to_comma($jenis[$k], '~');
							$data['bahanbaku']['b'][$k]['bentuk_bahanbaku']=$this->parse_to_comma($bentuk[$k], '~');
					...
					foreach($import as $k => $i){
						...
						if($i != ""){
							$data['bahanbaku']['i'][$k]['asal_bahanbaku']=$this->parse_to_comma($i, '~');
							$data['bahanbaku']['i'][$k]['jenis_bahanbaku']=$this->parse_to_comma($jenis[$k], '~');
							$data['bahanbaku']['i'][$k]['bentuk_bahanbaku']=$this->parse_to_comma($bentuk[$k], '~');
	...
	public function detail_skp($id,$ids)
	{
		...
		$data['upi']		= $this->model_upi->_get_upi_detail($data['skp'][0]['upi_id']);
	...
	public function delete_skp_pengajuan($idskp) {
		if($idskp != null && is_numeric($idskp) && $idskp > 0){
			// get data by idskp
			$skp_data = $this->model_skp->_get_skp($idskp);
			// $this->output->set_content_type('application/json')->set_output(json_encode($skp_data));
			// remove files
			if($skp_data[0]['filegmpssop_skp'] != ""){
				$this->delete_file($skp_data[0]['filegmpssop_skp']);
			}
			if($skp_data[0]['filesp_skp'] != ""){
				$this->delete_file($skp_data[0]['filesp_skp']);
			}
			// remove skp with dependencies
			$this->model_skp->_delete_skp_with_dependency($idskp);
			$this->nyast->notif_create_notification('Hapus SKP Berhasil','Selamat');
			redirect(site_url('skp/pengajuan_list'));
		} else {
			$this->nyast->notif_create_notification('Hapus SKP Gagal','Error');
			redirect(site_url('skp/pengajuan_list'));
		}
	}
